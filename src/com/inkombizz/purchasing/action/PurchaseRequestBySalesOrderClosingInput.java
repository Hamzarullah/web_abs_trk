...
@Results({
    @Result(name="success", location="purchasing/purchase-request-closing-input.jsp"),
    @Result(name="redirect", type="redirect", location="../pages/noauthority.jsp")
})

public class PurchaseRequestBySalesOrderClosingInput extends ActionSupport {
    ...
    public String execute() {
        try {
            ...
            if (!BaseSession.loadProgramSession().hasAuthority(purchaseRequestBLL.MODULECODE_CLOSING, EnumAuthorizationString.toString(EnumAuthorizationString.ENUM_AuthorizationString.UPDATE), hbmSession)) {
                return "redirect";
            }            
            
            purchaseRequestClosing = (PurchaseRequestBySalesOrder) hbmSession.hSession.get(PurchaseRequestBySalesOrder.class, purchaseRequestClosing.getCode());
